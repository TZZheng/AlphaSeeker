...
runtime side of the future frontend harness: it sends the request to the
existing ``<TICKER>_orchestrator``. Existing long-lived agents are not
re-prompted on every request; they keep their own memory, pad, and habits.
...
existing ``.lingtai/<TICKER>_<role>/init.json`` files with
``--apply-comments``.
...
def init_json_path(root: Path, *, ticker: str, role: str) -> Path:
    return root / ".lingtai" / agent_name(ticker, role) / "init.json"
...
def ensure_vault_dirs(root: Path, *, ticker: str) -> list[Path]:
    base = root / "vault" / "companies" / ticker.upper()
    paths = [
        base / "wiki",
        base / "team" / "raw",
        base / "team" / "drafts",
...
def _human_identity(root: Path) -> dict[str, object]:
    path = root / ".lingtai" / "human" / ".agent.json"
    if path.exists():
        data = json.loads(path.read_text(encoding="utf-8"))
        return dict(data)
    return {"address": "human", "agent_name": "human", "nickname": "Terry"}
...
def enqueue_request(root: Path, *, ticker: str, request: str, subject: str = "") -> Path:
    ...
    recipient_dir = root / ".lingtai" / recipient
    if not recipient_dir.exists():
        raise SystemExit(f"missing orchestrator agent directory for {recipient}: {recipient_dir}")

    outbox = root / ".lingtai" / "human" / "mailbox" / "outbox"
...
def main() -> int:
    ...
    parser.add_argument("--root", default=".", help="AlphaSeeker repository root. Default: current directory.")
    parser.add_argument(
        "--role",
        default="all",
    ...
    parser.add_argument("--send", action="store_true", help="Runtime mode: queue the request to <TICKER>_orchestrator via human outbox.")
    ...
    if not (args.send or args.apply_comments or args.render_comments or args.ensure_dirs):
        raise SystemExit("nothing to do; use --send for an existing team, or --render-comments/--apply-comments for setup")
