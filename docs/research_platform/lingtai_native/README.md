...
The first manual v0 run had no new tools, no panel, no setup CLI, and no in-process AlphaSeeker team runtime. After the TSLA run, the first formalized additions are source-data tools and a full-investment-conclusion orchestrator steering design; LingTai still owns the runtime.
...
- `automatic_orchestrator_questions.md` — general steering loop that targets full investment conclusion and triggers the smallest concrete evidence action.
