...
- Ensure the final accepted output is written to `published/latest.md` only after the publication judgment is satisfied or the remaining non-yes gates have explicit stopping reasons. Do not publish merely because a weaker conclusion is already defensible if a feasible decision-relevant step remains.
...
This is a publication gate, not a loose checklist. Before publishing, answer these questions in natural language:
...
Every gate must be satisfied, or you must explain why the team cannot make it satisfied now. If any answer is not effectively yes, first try to make it yes through the appropriate source/writer/reviewer cycle. Publish only when the gates are satisfied, or when the remaining non-yes answers cannot be turned into yes because they are infeasible, outside scope, low materiality, too costly for likely value, or unlikely to change the decision. In that case, publish only with the limitation and stopping reason stated plainly.

Do not turn this into a fixed DCF/peer/model checklist. Let the needed work emerge from this ticker, conclusion, evidence base, price, and risk/reward. Do not rely on the team's self-set stopping criteria to lower the bar. Use them as context, but judge against the human's original objective.
